...
<div class="d-flex justify-content-between flex-wrap flex-md-nowrap align-items-center pt-2 pb-3 mb-4 border-bottom" data-aos="fade-down">
  <div>
    <h2 class="h3 fw-bold text-dark mb-0"><i class="fas fa-hotel me-2"></i>Fasilitas Hotel</h2>
      ...
      <table class="table table-striped table-bordered table-hover align-middle datatable w-100">
        ...
        <tbody>
          ...
          <tr>
            ...
            <!-- POIN 6 FIX: Default Tanda Strip (-) Jika Deskripsi Kosong -->
            <td>
              @php
                $deskripsiBersih = trim(strip_tags($row->deskripsi));
              @endphp
              
              @if(!empty($deskripsiBersih) && $deskripsiBersih !== '-')
                {!! $row->deskripsi !!}
              @else
